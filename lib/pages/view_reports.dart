...
class _view_reportsState extends State<view_reports> {
  ...
  Future<void> signOut()async{
    await auth().signOut();
    CoolAlert.show(context: context, type: CoolAlertType.success,width:20.0,text: "You have logged out ",);
  }
  Widget _userid(){
  ...
  Widget build(BuildContext context) {
    ...
    builder: (context,snapshot) {
      ...
      return Center(child: CircularProgressIndicator(color: color_palette().light,));
    }
                  ),
                ),
              ...
              Positioned(child: signOutButton())
                  ]
          );
  ...
  get_data()async{
    ...
    CoolAlert.show(context: context, type: CoolAlertType.info,width:20.0,text: "There are $a new alerts",title:"Update");
    backref.doc("reportCount").update({
      "alerts":0
    });
    return allData;
  }
  Widget tile(i){
    ...
    String ph=a["ph"];
    return Container(
      padding: EdgeInsets.symmetric(vertical:global_var.height*0.05,horizontal:global_var.width*0.08),
    ...
    return Column(
      children: [
        Row(
          ...
          children: [
            Text("$label : ",style: color_palette().report_label_style,),
            Flexible(child: Text("$data",style: color_palette().report_data_style,textAlign: TextAlign.right,)),
          ],
        ),
        SizedBox(height: 10,)
      ],
